...
| `make disk` | an ADFS "D" format (800KB) disc image (`build/ArchiLudo-vX.Y.Z-<timestamp>.adf`) containing a leaner zip than `make zip`'s own -- the app directory and plain-text `ReadMe,fff` only, not `README.pdf` (several MB once it embeds every "Interface manual" screenshot, far beyond any floppy's capacity, and RISC OS 3.10 has no PDF viewer to read it with anyway), filetyped `&A91` (Zip). Written by `tools/build_adfs_disk.py` -- see [TOOLS.md](TOOLS.md) |
